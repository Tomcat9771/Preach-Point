...
export default function App() {
  ...
  useEffect(() => {
    ...
    setChapterStart(''); setVerseStart(''); setChapterEnd(''); setVerseEnd('')
    setTone(toneOptions[lang][0]); setLevel(levelOptions[lang][0])
    setPassage(''); setCommentary('')
  }, [lang])
  ...
  useEffect(() => {
    ...
    fetch(`/api/chapters?book=${encodeURIComponent(bookName)}`)
      ...
      .then(js => { setChapters(js.chapters || []); setChapterStart(''); setChapterEnd(''); setVerses([]) })
      .catch(() => setChapters([]))
  }, [bookIdx])
  ...
  useEffect(() => {
    ...
    fetch(`/api/versesCount?book=${encodeURIComponent(bookName)}&chapter=${chapterStart}`)
      .then(res => { if (!res.ok) throw new Error(); return res.json() })
      .then(js => { setVerses(js.verses || []); setVerseStart(''); setVerseEnd('') })
      .catch(() => setVerses([]))
  ...
  return (
    <div className="container" ref={printRef}>
      ...
      <div id="output">
        {passage && <><h2>{HD.verses}</h2><div>{passage}</div></>}
        {commentary && <><h2>{HD.commentary}</h2><div>{commentary}</div><div className="actions"><button onClick={handleCopy}>{BTN.copy}</button><button onClick={handleDownload}>{BTN.pdf}</button></div></>}
      </div>
    </div>
  )
}
